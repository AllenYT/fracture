...
class MyQueuePanel extends Component {
    ...
    this.moveRight = this
        .moveRight
        .bind(this)
    this.moveAllRight = this
        .moveAllRight
        .bind(this)
    this.removeRight = this
        .removeRight
        .bind(this)
    this.submitQueue = this 
        .submitQueue
        .bind(this)
    ...
            if(leftselect.options[i].selected){
                for(var k=0;k<rightpidList.length;k++){
                    if(rightpidList[k] === leftselect[i].text)
                        isExist = true
                }
                if(!isExist){
    ...
    moveAllRight(){
        ...
        for(var i=0;i<leftpidList.length;i++){
            for(var j=0;j<rightpidList.length;j++){
                if(rightpidList[j] === leftpidList[i])
                    isExist = true
            }
            if(!isExist){
    ...
    RemoveDuplicate(list){
        ...
            var s = list[i];
            if (obj[s]) continue;//如果字du符串已经存在就跳过zhi
            else {
                obj[s] = s;//加入标记对象中
                arr.push(s);//结果放入新数组中
            }
                   ...
                   <Grid>
                       <Grid.Row>
                           ...
                           <Grid.Column width={10}>
                                <Modal open={open} trigger={<Button inverted color='blue' title='创建队列'
                                icon><Icon name='plus'></Icon></Button>} size='large' closeIcon onOpen={this.open}
                                onClose={this.close}>
                                        ...
                                        <Grid.Row>
                                            ...
                                            <Grid.Row centered>
                                                ...
                                                <Grid.Column width={1} verticalAlign='middle' textAlign='center'>
                                                    <Button icon onClick={this.moveRight}><Icon name='angle right'></Icon></Button>
                                                    <Button icon onClick={this.moveAllRight}><Icon name='angle double right'></Icon></Button>
                                                    <Button icon onClick={this.removeRight}><Icon name='trash'></Icon></Button>
                                                </Grid.Column>
                                                <Grid.Column width={7} verticalAlign='middle' textAlign='center'>
                        ...
                        <div class="queue-name">
                            <Grid>
                                <Grid.Row>
                                    ...
                                    <Grid.Column width={3}>
                                        <Button id="header-right" onClick={this.hidder} icon><Icon name='x'></Icon></Button> 
                                    </Grid.Column>
