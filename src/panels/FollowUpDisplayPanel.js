...
class FollowUpDisplayPanel extends Component {
  constructor(props) {
    super(props);
    this.state = {
      curCaseId: window.location.pathname
        .split("/followup/")[1]
        .split("/")[0]
        .split("&")[0],
      preCaseId: window.location.pathname
        .split("/followup/")[1]
        .split("/")[0]
        .split("&")[1],
  ...
  async componentWillMount() {
    ...
    if (this.state.username === "origin") {
      ...
      axios
        .post(
          this.config.draft.getRectsForFollowUp,
          qs.stringify(followRectsParams)
        )
