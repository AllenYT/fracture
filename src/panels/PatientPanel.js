...
class PatientPanel extends Component {
  constructor(props) {
    ...
    this.toCase = this.toCase.bind(this);
        ...
        if (data.status !== "okay") {
          ...
          for (let key in subList) {
            ...
            for (let j = 0; j < seriesLst.length; j++) {
              theList.push({
                date: seriesLst[j]["date"],
                caseId: seriesLst[j]["caseId"],
                href: "/case/" + seriesLst[j]["caseId"] + "/origin",
              });
            }
          }
  ...
  componentDidMount() {
    ...
    axios
      .post(this.config.record.getPatientInfo, qs.stringify(params))
      .then((res) => {
        const data = res.data;
        this.setState({
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      prevState.pidKeyword !== this.state.pidKeyword ||
      prevState.dateKeyword !== this.state.dateKeyword ||
  ...
  getTotalPages() {
    ...
    axios
      .post(this.config.record.getTotalPages, qs.stringify(params), { headers })
      .then((response) => {
        const data = response.data;
        if (data.status !== "okay") {
          alert("错误，请联系管理员");
  ...
  render() {
    ...
    for (var i = 0; i < record.length; i++) {
      diagnoisisTable.push(
        <Table.Row key={i}>
          <Table.Cell>{record[i].caseId}</Table.Cell>
    ...
    if (window.location.pathname.split("/")[2] !== undefined) {
            ...
            <Grid.Row stretched>
              <Grid.Column width="3">
                <Menu pointing secondary vertical>
                  <div id="menu-header">
                    <Menu.Item header>
                      <p style={{ color: "white", fontSize: 22 + "px" }}>
                        病人详情
                      </p>
                    </Menu.Item>
                  </div>
              ...
              <Grid.Column width="12">
                <div id="informationPanel">
                  <div className="information-header">
                    <Grid.Row stretched>
                      基本信息&nbsp;
                      <p style={{ fontSize: 12 + "px", textAlign: "end" }}>
                    ...
                    <Grid.Row>
                      <Grid verticalAlign="middle">
                        ...
                        <Grid.Row columns={2} textAlign="left">
                          ...
                          <Grid.Column width="4">
                            <p style={headerStyle}>记录结节最大直径:&nbsp;</p>
                            {Math.round(this.state.maxDiameter)}
                          </Grid.Column>
                        </Grid.Row>
                      </Grid>
                    </Grid.Row>
                ...
                <div id="diagnoisis">
                  ...
                  <Grid.Row>
                    ...
                    <Table inverted singleLine id="dianogsis-table">
                      ...
                      <Table.Body>{diagnoisisTable}</Table.Body>
                    </Table>
                    {/* </div> */}
      ...
      return (
        <div>
          <h2 style={noPatientSelectHintStyle}>
            当前并未选中任何病人，请前往主页面选择一位病人以查看信息
          </h2>
        </div>
      );
    }
