...
function getTangent(pointLast, pointNext) {
  return vec3.fromValues((pointLast[0] - pointNext[0]) / 2, (pointLast[1] - pointNext[1]) / 2, (pointLast[2] - pointNext[2]) / 2)
}
...
function getNormal(tangentLast, tangentNext) {
  const normal = vec3.fromValues(tangentNext[0] - tangentLast[0], tangentNext[1] - tangentLast[1], tangentNext[2] - tangentLast[2])
...
function rotateVector(vector, axis, angle) {
  ...
  for (let i = 0; i < 3; i++) {
    result[i] = Math.cos(angle) * vector[i] + (1 - Math.cos(angle)) * dot * axis[i] + Math.sin(angle) * vec[i]
  }
  return result
}
