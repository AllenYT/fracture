  ...
  render() {
    ...
    for (const subKey in subList) {
      ...
      for (let studyitem in studyAry) {
        ...
        let patientName = ''
        let patientSex = study['gender'] === '' ? '' : study['gender'] === 'M' ? '男' : '女'
        let newValue = []
        if (this.props.type === 'date') {
          ...
          <div key={idx}>
            <Accordion.Title className="space" active={this.state.activeIndex === idx} index={idx} onClick={this.handleClick}>
              ...
              <div
                ...
                {/* <span className="display-right">共{len}次检查</span> */}共{len}次检查
              </div>
              {/* <tr style={{ width: "100%" }}>
                <td>
                  <Icon name={icon} />
        ...
        <div>
          <div className="hint">
            ...
            {panels.length !== 0 && icon === 'calendar' ? (
              <Button style={patientInfoButtonStyle} content="病人详情" icon="right arrow" labelPosition="right" className="ui green inverted button" onClick={this.handlePidClick} />
            ) : null}
          </div>
