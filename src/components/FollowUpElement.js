...
class FollowUpElement extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      activeTool: 'Wwwc',
      initialViewport: props.stack.initialViewport === '' ? cornerstone.getDefaultViewport(null, undefined) : props.stack.initialViewport,
      random: Math.random(),
    }
  ...
  componentDidMount() {
    ...
    Promise.all([curImagePromise, preImagePromise]).then(() => {
      console.log('this.props', cornerstone.imagePoolLoadManager)
    })
    console.log('this.imagePoolLoadManager', cornerstone.imagePoolLoadManager)
    var templateText = ''
  ...
  render() {
        ...
        <div key={idx}>
          <Accordion.Title index={idx} onClick={this.handleListClick.bind(this, inside.slice_idx, idx, 'current')} active={curListsActiveIndex === idx} className="current-nodule-accordion-title">
            <Row gutter={1} justify="space-around">
              ...
              <Col span={5}>
                定位：
                <Cascader options={this.config.segment} onChange={this.onLungLocationChange.bind(this, idx, 'current')}>
                  <a href="#">{lungLoc[inside.segment]}</a>
                </Cascader>
              </Col>
              <Col span={3}>
                <Cascader options={this.config.dangerLevel} onChange={this.onDangerLevelChange.bind(this, idx, 'current')}>
        ...
        <div key={idx}>
          <Accordion.Title index={idx} onClick={this.handleListClick.bind(this, inside.slice_idx, idx, 'previews')} active={preListsActiveIndex === idx} className="current-nodule-accordion-title">
            <Row gutter={1} justify="space-around">
              ...
              <Col span={5}>
                定位：
                <Cascader options={this.config.segment} onChange={this.onLungLocationChange.bind(this, idx, 'previews')}>
                  <a href="#">{lungLoc[inside.segment]}</a>
                </Cascader>
              </Col>
              <Col span={3}>
                <Cascader options={this.config.dangerLevel} onChange={this.onDangerLevelChange.bind(this, idx, 'previews')}>
                  <a href="#">{dangerLevel[inside.malignancy]}</a>
                </Cascader>
              </Col>
              <Col span={3}>
    ...
    if (matchNoduleLen !== 0) {
      matchNodulesTbl = registerBoxes['match'].map((value, idx) => {
        ...
        return (
          <Row key={idx} justify="center" className="register-nodule-match-card" onClick={this.onMatchNoduleChange.bind(this, newNodule, previewsNodule)}>
            <Col span={1}>
    ...
    if (newNoduleLen !== 0) {
      newNodulesTbl = registerBoxes['new'].map((value, idx) => {
        ...
        return (
          <Row key={idx} justify="center" className="register-nodule-new-card" onClick={this.onNewNoduleChange.bind(this, value)}>
            <Col span={1}>
            ...
            <Col span={22}>
              <Row>
                <Col span={24}>
                  定位：
                  <Cascader options={this.config.segment} onChange={this.onLungLocationChange.bind(this, idx, 'register-new')}>
              ...
              <Row>
                ...
                <Col span={5}>
                  密度：
                  <Cascader options={this.config.densityConfig} onChange={this.onDensityChange.bind(this, idx, 'register-new')}>
    ...
    if (vanishNoduleLen !== 0) {
      vanishNodulesTbl = registerBoxes['vanish'].map((value, idx) => {
        ...
        return (
          <Row key={idx} onClick={this.onPreNoduleChange.bind(this, value)} justify="center" className="register-nodule-previews-card">
            <Col span={1}>
              <Row></Row>
            ...
            <Col span={22}>
              <Row>
                <Col span={24}>
                  定位：
                  <Cascader options={this.config.segment} onChange={this.onLungLocationChange.bind(this, idx, 'register-vanish')}>
                    <a href="#">{lungLoc[value.segment]}</a>
                  </Cascader>
                </Col>
              </Row>
              ...
              <Row>
                ...
                <Col span={5}>
                  密度：
                  <Cascader options={this.config.densityConfig} onChange={this.onDensityChange.bind(this, idx, 'register-vanish')}>
                    <a href="#">{densityList[value.texture]}</a>
                  </Cascader>
                </Col>
                <Col span={8}>
        ...
        <Menu className="corner-header">
          ...
          <Menu.Item className="funcolumn">
            <Button.Group>
              ...
              <Button icon title="放大" onClick={this.ZoomIn.bind(this)} className="funcbtn">
                <Icon name="search plus" size="large"></Icon>
              </Button>
              <Button icon title="缩小" onClick={this.ZoomOut.bind(this)} className="funcbtn">
                <Icon name="search minus" size="large"></Icon>
              </Button>
              <Button icon onClick={this.reset.bind(this)} className="funcbtn" title="刷新">
        ...
        {this.state.isRegistering === false ? (
          <div>
            <Row justify="space-around" className="BoxesAccord-Row">
              <Col span={12}>
                <Accordion className="current-nodule-accordion">{curBoxesAccord}</Accordion>
              </Col>
              <Col span={12}>
                <Accordion className="current-nodule-accordion">{preBoxesAccord}</Accordion>
              </Col>
            </Row>
          </div>
        ) : (
