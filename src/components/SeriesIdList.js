...
class SeriesIdList extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    console.log("contentParent", this.props.content);
  ...
  render() {
    ...
    resultsPopup = content.map((value, index) => {
      const idName = value.caseId + "_" + index;
      var popupContent = "";
      ...
      for (let i = 0; i < dataValidContnt.length; i++) {
        if (dataValidContnt[i].caseId === value["caseId"]) {
          console.log("i", i, dataValidContnt[i].caseId, value["caseId"]);
      ...
      return (
        ...
        >
          ...
          >
            ...
            <div className="sid">{value["description"]}</div>
            {statusIcon}
          </div>
          <div style={{}}>
            <Tooltip
