...
class StudyBrowserList extends Component {
  ...
  componentDidMount() {
    ...
    //   // console.log("validContent", validContent);
    // });
    // console.log('test study', dateSeries)
    // console.log('test study', dateSeries[1])
    // console.log('test study', dateSeries.length)
    // for (let i = 0; i < dateSeries.length; i++) {
    //   console.log('test study', dateSeries[i].caseId)
    //   axios
    //     .post(
    //       this.config.draft.dataValid,
    //       qs.stringify({
    ...
    return (
      <div className="preview">
        {dateSeries.map((serie, index) => {
          ...
          return (
            <Card onClick={(e) => this.props.handleClickScreen(e, serie.href, validStatus)} key={keyId}>
