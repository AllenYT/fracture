...
class CornerstoneElement extends Component {
  ...
  visualize(hist_data, idx) {
    ...
    if (
      document.getElementById(visId) &&
      document.getElementById("closeVisualContent")
    ) {
      const histogramClientHeight = document.getElementById(visId).clientHeight;
      const closeClientHeight =
        document.getElementById("closeVisualContent").clientHeight;
      this.setState({
                  ...
                  >
                    ...
                    >
                      ...
                      <button
                        ...
                        style={{ bottom: `${histogramHeight}px` }}
  ...
  resizeScreen(e) {
    ...
    const verticalMode =
      document.body.clientWidth < document.body.clientHeight ? true : false;
    this.setState({
      windowWidth: document.body.clientWidth,
      windowHeight: document.body.clientHeight,
      verticalMode,
        ...
        () => {
          if (this.state.show3DVisualization) {
            ...
            if (document.getElementById("threed-mask-container")) {
              const threedMaskContainer = document.getElementById(
                "threed-mask-container"
              );
              this.setState({
    ...
    if (
      document.getElementById(`visual-${this.state.listsActiveIndex}`) &&
      document.getElementById("closeVisualContent")
    ) {
    ...
    if (type === "影像所见") {
      ...
      if (activeItem === -1) {
        ...
        if (boxes[activeItem]["measure"] !== undefined) {
          ...
          if (ll === 0 && sl === 0) {
            if (
              this.props.boxes[activeItem]["diameter"] !== undefined &&
              this.props.boxes[activeItem]["diameter"] !== 0
            ) {
              diameter =
                "\xa0\xa0" +
                (this.props.boxes[activeItem]["diameter"] / 10).toFixed(2) +
                " 厘米";
            } else {
              diameter = "未知";
            }
  ...
  getMPRInfo(imageIds) {
    ...
    Promise.all(promises).then(() => {
      ...
      const onAllPixelDataInsertedCallback = () => {
        const { actor, mapper } = this.createActorMapper(
  ...
  setActive(classfication, index, urlIndex, e) {
    ...
    if (classfication === 0) {
      ...
      tubularController.tubularActive[index] =
        !tubularController.tubularActive[index];
      this.setState({
        tubularController,
      });
    ...
    if (classfication === 0) {
      ...
      if (tubularController.tubularVisible[index]) {
        this.setSegmentOpacity(
          urlIndex,
          tubularController.tubularOpacities[index]
        );
      } else {
        this.setSegmentOpacity(urlIndex, 0);
      }
  ...
  setOpacityChangeable(classfication, index, e) {
    ...
    if (classfication === 0) {
      ...
      const tubularController = this.state.tubularController;
      tubularController.tubularOpacityChangeable[index] =
        !tubularController.tubularOpacityChangeable[index];
      this.setState({
        tubularController,
      });
  ...
  changeMode(mode) {
    ...
    this.setState(
      ...
      () => {
        ...
        if (mode === 2) {
          const MPRStyles = this.getMPRStyles();
          this.viewerAxial.setContainerSize(
            MPRStyles.axial.width,
            MPRStyles.axial.height
          );
          this.viewerCoronal.setContainerSize(
            MPRStyles.coronal.width,
            MPRStyles.coronal.height
          );
          this.viewerSagittal.setContainerSize(
            MPRStyles.sagittal.width,
            MPRStyles.sagittal.height
          );
        }
        if (mode === 3) {
          const CPRStyles = this.getCPRStyles();
          this.viewerAxial.setContainerSize(
            CPRStyles.axial.width,
            CPRStyles.axial.height
          );
          this.viewerCoronal.setContainerSize(
            CPRStyles.coronal.width,
            CPRStyles.coronal.height
          );
          this.viewerSagittal.setContainerSize(
            CPRStyles.sagittal.width,
            CPRStyles.sagittal.height
          );
  ...
  createNoduleMask(idx) {
    ...
    if (!labelData) {
      const segmentIndex = _.findIndex(this.state.urls, {
        class: 2,
        order: this.state.nodules[idx].prevIdx + 1,
      });
      ...
      let minX =
        Math.round(Math.min(firstOriginIndex[0], lastOriginIndex[0])) - 20;
      let maxX =
        Math.round(Math.max(firstOriginIndex[0], lastOriginIndex[0])) + 20;
      let minY =
        Math.round(Math.min(firstOriginIndex[1], lastOriginIndex[1])) - 20;
      let maxY =
        Math.round(Math.max(firstOriginIndex[1], lastOriginIndex[1])) + 20;
      ...
      const dimensions = this.state.dimensions;
      const scalarsData = this.state.vtkImageData
        .getPointData()
        .getScalars()
        .getData();
      const indices = [];
      ...
      for (let y = minY; y < maxY; y++) {
        for (let x = minX; x < maxX; x++) {
          newPixelArray[x - minX + (y - minY) * xLength] =
            scalarsData[
              x + y * dimensions[0] + (z - 1) * dimensions[0] * dimensions[1]
            ];
          if (
            newPixelArray[x - minX + (y - minY) * xLength] >
            this.state.labelThreshold - 1024
          ) {
            if (
              x >= range.minX &&
              x < range.maxX &&
      ...
      const labelMapPixelArray = new Float32Array(xLength * yLength * 5).fill(
        -1024
      );
      indices.forEach((item) => {
