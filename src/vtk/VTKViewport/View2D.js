...
export default class View2D extends Component {
  ...
  updateSliderValue(value) {
    value = Math.round(value - this.props.sliderMin)
    this.setState({
      sliderValue: value,
    })
  }
  ...
  componentDidMount() {
    ...
    this.throttledUpdateRotationOverlay = throttle(this.updateRotationOverlay, 16, { trailing: true }) // ~ 60 fps
    ...
    const boundResetAllView = this.resetAllView.bind(this)
    const boundUpdatePaintbrush = this.updatePaintbrush.bind(this)
    const boundUpdateSliderValue = this.updateSliderValue.bind(this)
    ...
    if (this.props.onCreated) {
      ...
      const api = {
        ...
        resetAllView: boundResetAllView,
        updatePaintbrush: boundUpdatePaintbrush,
        updateSliderValue: boundUpdateSliderValue,
    ...
    if (istyle.setSlabThickness) {
      ...
      if (this.props.paintFilterLabelMapImageData) {
        ...
        if (realsApproximatelyEqual(slabThickness, minSlabThickness)) {
          if (labelmapActor.getVisibility() !== this.props.labelmapRenderingOptions.visible) {
            labelmapActor.setVisibility(this.props.labelmapRenderingOptions.visible)
          }
        } else {
  ...
  render() {
    ...
    const sliderLength = sliderMax - sliderMin
    const slider = sliderLength ? (
      <>
        <Slider
          style={sliderStyle}
          vertical
          ...
          onAfterChange={this.afterChangeSlice.bind(this)}
        />
      </>
    ) : (
      <></>
