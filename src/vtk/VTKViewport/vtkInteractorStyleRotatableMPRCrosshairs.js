...
function vtkInteractorStyleRotatableMPRCrosshairs(publicAPI, model) {
  ...
  function selectOpperation(callData) {
    ...
    if (!rotatableCrosshairsWidget) {
      throw new Error('Must use rotatable crosshair svg widget with this istyle.')
    }
  ...
  function performOperation(callData) {
    ...
    switch (type) {
      case operations.MOVE_CROSSHAIRS:
        // console.log("positon", callData.position, "scale", scale)
      ...
      case operations.MOVE_REFERENCE_LINE:
        ...
        // console.log("positon", callData.position, "pos", pos, "scale", scale)
  ...
  function rotateCrosshairs(callData) {
    ...
    vec2.subtract(pointToPreviousPosition, [prevPosition.x, prevPosition.y], point)
    ...
    const determinant = pointToNewPosition[0] * pointToPreviousPosition[1] - pointToNewPosition[1] * pointToPreviousPosition[0]
    ...
    apis.forEach((api, index) => {
      if (index !== apiIndex) {
        const cameraForApi = api.genericRenderWindow.getRenderWindow().getInteractor().getCurrentRenderer().getActiveCamera()

        const crosshairPointForApi = api.get('cachedCrosshairWorldPosition')
        const initialCrosshairPointForApi = api.get('initialCachedCrosshairWorldPosition')
        ...
        const { matrix } = vtkMatrixBuilder.buildFromRadian().translate(translate[0], translate[1], translate[2]).rotate(angle, axis).translate(-translate[0], -translate[1], -translate[2])
    ...
    */
    ...
    apis.forEach((api) => {
      ...
      cameraForApi.setFocalPoint(newFocalPointForApi[0], newFocalPointForApi[1], newFocalPointForApi[2])
    })
  ...
  function snapPosToLine(position, lineIndex) {
    ...
    const newCenterSVG = [points[0].x + unitVectorAlongLine[0] * distanceAlongLine, points[0].y + unitVectorAlongLine[1] * distanceAlongLine]
  ...
  function moveCrosshairs(pos, renderer) {
    ...
    thisApi.svgWidgets.rotatableCrosshairsWidget.moveCrosshairs(worldPos, apis, apiIndex)
  ...
  function scrollCrosshairs(lineIndex, direction) {
    ...
    const unitVector = []
    vec2.subtract(unitVector, [lowToHighPoints[1].x, lowToHighPoints[1].y], [lowToHighPoints[0].x, lowToHighPoints[0].y])
    ...
    const displayCoordintateScrollIncrement = getDisplayCoordinateScrollIncrement(point)

    const newCenterPointSVG = [p[0] + unitVector[0] * displayCoordintateScrollIncrement, p[1] + unitVector[1] * displayCoordintateScrollIncrement]
  ...
  function getDisplayCoordinateScrollIncrement(point) {
    ...
    const distanceOfOnePixelInWorld = vec2.distance(worldPosCenter, worldPosOnePixelOver)
...
export function extend(publicAPI, model, initialValues = {}) {
  ...
  macro.setGet(publicAPI, model, ['callback', 'apis', 'apiIndex', 'onScroll', 'operation', 'lineGrabDistance', 'disableNormalMPRScroll'])
