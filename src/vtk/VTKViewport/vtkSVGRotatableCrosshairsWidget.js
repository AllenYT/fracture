...
function vtkSVGRotatableCrosshairsWidget(publicAPI, model) {
  ...
  model.calculateReferenceLines = (apiIndex, point) => {
    ...
    const crosshairWorldPosition = apis[apiIndex].get('cachedCrosshairWorldPosition')
    ...
    for (let i = 0; i < apis.length; i++) {
      if (i !== apiIndex) {
        ...
        const doubleSVGPosition = [doubleDisplayPosition[0] * scale, height - doubleDisplayPosition[1] * scale]
        ...
        const distantPoint = [p[0] + unitVectorFromCenter[0] * farDistance, p[1] + unitVectorFromCenter[1] * farDistance]

        const negativeDistantPoint = [p[0] - unitVectorFromCenter[0] * farDistance, p[1] - unitVectorFromCenter[1] * farDistance]
        ...
        const oldReferenceLine = oldReferenceLines.find((refLine) => refLine && refLine.apiIndex === i)
    ...
    rotatableCrosshairsWidget.setReferenceLines(referenceLines[0], referenceLines[1])
  }

  publicAPI.render = (svgContainer, scale) => {
    const node = getWidgetNode(svgContainer, model.widgetId)
    let { point, strokeColors, strokeWidth, strokeDashArray, rotateHandleRadius, apis, apiIndex, selectedStrokeWidth } = model
    if (point[0] === null || point[1] === null) {
      return
    }
    ...
    const { points: firstLineRotateHandles, selected: firstLineRotateSelected } = firstLine.rotateHandles
    const { points: secondLineRotateHandles, selected: secondLineRotateSelected } = secondLine.rotateHandles

    const [firstLinePart1, firstLinePart2] = model.getSplitReferenceLine(firstLine, p)
    const [secondLinePart1, secondLinePart2] = model.getSplitReferenceLine(secondLine, p)

    const firstLineStrokeColor = strokeColors[firstLine.apiIndex]
    const secondLineStrokeColor = strokeColors[secondLine.apiIndex]

    const firstLineStrokeWidth = firstLine.selected || firstLine.active ? selectedStrokeWidth : strokeWidth
    const secondLineStrokeWidth = secondLine.selected || secondLine.active ? selectedStrokeWidth : strokeWidth

    const firstLineRotateWidth = firstLineRotateSelected ? selectedStrokeWidth : strokeWidth
    const secondLineRotateWidth = secondLineRotateSelected ? selectedStrokeWidth : strokeWidth

    const firstLineShowCrosshairs = firstLine.selected || firstLineRotateSelected
    const secondLineShowCrosshairs = secondLine.selected || secondLineRotateSelected

    const firstLineRotateHandleRadius = firstLineShowCrosshairs ? rotateHandleRadius : 0

    const secondLineRotateHandleRadius = secondLineShowCrosshairs ? rotateHandleRadius : 0

    const firstLineRotateHandleFill = firstLineRotateSelected ? referenceLines[0].color : 'none'

    const secondLineRotateHandleFill = secondLineRotateSelected ? referenceLines[1].color : 'none'
    ...
    if (model.display) {
      ...
      <circle cx="${width - 20}" cy="${20}" r="10" fill="${strokeColors[apiIndex]}" />
      </g>
  ...
  model.getSplitReferenceLine = (referenceLine, center) => {
    ...
    const lineDirection = []
    vec2.subtract(lineDirection, [referenceLine.points[1].x, referenceLine.points[1].y], [referenceLine.points[0].x, referenceLine.points[0].y])
  ...
  publicAPI.moveCrosshairs = (worldPos, apis) => {
    ...
    apis.forEach((api, apiIndex) => {
      ...
      switch (apiIndex) {
        case 0:
          api.updateSliderValue(worldPos[2])
          break
        case 1:
          api.updateSliderValue(worldPos[1])
          break
        case 2:
          api.updateSliderValue(worldPos[0])
          break
        default:
          break
      }
      ...
      api.svgWidgets.rotatableCrosshairsWidget.setPoint(displayPosition[0], displayPosition[1])
...
export function extend(publicAPI, model, initialValues = {}) {
  ...
  macro.setGet(publicAPI, model, ['strokeWidth', 'selectedStrokeWidth', 'strokeDashArray', 'display', 'apiIndex', 'referenceLines', 'centerRadius'])
...
export const newInstance = macro.newInstance(extend, 'vtkSVGRotatableCrosshairsWidget')
